...
export default function PriceChart({ prices, ticker }: PriceChartProps) {
  ...
  const toY = (v: number) =>
    PAD.top + innerH - ((v - minClose) / range) * innerH;
  ...
  const isUp =
    prices.length >= 2 && closes[closes.length - 1] >= closes[0];
  const lineColor = isUp ? "#10b981" : "#ef4444";
  const gradId = `grad-${ticker.replace(/\./g, "_")}`;
        ...
        <div style={{ display: "flex", gap: "12px", fontSize: "0.8rem" }}>
          <span style={{ color: "var(--text-secondary)" }}>
            ...
            <span style={{ color: "#10b981", fontWeight: 600 }}>
              ¥{maxClose.toLocaleString("ja-JP")}
            </span>
          </span>
          <span style={{ color: "var(--text-secondary)" }}>
            安値:{" "}
            <span style={{ color: "#ef4444", fontWeight: 600 }}>
              ¥{minClose.toLocaleString("ja-JP")}
            </span>
          </span>
        </div>
      </div>
        ...
        >
          ...
          {yTicks.map((v, i) => (
            <text
              ...
              textAnchor="end"
              fontSize="10"
              fill="rgba(139,154,184,0.7)"
            >
              {v >= 1000 ? `${(v / 1).toFixed(0)}` : v.toFixed(2)}
            </text>
          ))}
          ...
          {xLabels.map((idx) => (
            ...
            >
              {prices[idx]?.date
                ? new Date(prices[idx].date).toLocaleDateString("ja-JP", {
                    month: "numeric",
                    day: "numeric",
                  })
                : ""}
